...
// Замени на свои данные! Создай .env файл в папке server:
// DATABASE_URL=postgres://username:password@localhost:5432/quiz_platform
const pool = new Pool({
...
// In-memory active sessions (для скорости real-time)
let activeSessions = new Map(); // roomCode -> session object
...
// Получить один квиз с вопросами (для редактирования)
...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', async ({ roomCode, userId, userName, isOrganizer }) => {
    ...
    if (!session) {
      // Пытаемся восстановить из БД (если сервер перезапустили)
    ...
    // Если квиз уже идёт — отправляем текущее состояние ТОЛЬКО новому участнику
    ...
    // Если finished — отправляем лидерборд
  ...
  socket.on('submit-answer', async ({ roomCode, userId, questionId, answerIds }) => {
    ...
    if (!participant || participant.hasAnsweredCurrent) return; // защита от повторного ответа
    ...
    participant.hasAnsweredCurrent = true; // помечаем что ответил на текущий
